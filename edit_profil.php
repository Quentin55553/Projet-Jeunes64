<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $age = $birthDate->diff($currDate);

        $ageInYears = $age->format('%Y');

        if ($ageInYears < 16 || $ageInYears > 30) {
            // Si l'âge de l'utilisateur n'est pas compris entre 16 et 30 inclus
            $message = "<div class='alert alert-warning alert-white rounded'>
                            <div class='icon'><i class='fa fa-warning'></i></div>
                            <strong>Attention !</strong> Vous devez obligatoirement avoir entre 16 et 30 ans.
                        </div>";
                        
        } else if (($email != $email_session) && (array_key_exists($email, $users))) { // Vérifie si le nouveau mail n'est pas déjà associé à un compte
            $message = "<div class='alert alert-warning alert-white rounded'>
                            <div class='icon'><i class='fa fa-warning'></i></div>
...
<html lang="fr">
    <head>
        ...
        <title>Jeunes 6.4 - Modification profil</title>
