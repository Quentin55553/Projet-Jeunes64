<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($consultEmail == $_SESSION['email']) {
            $send_msg = "<div class='alert alert-warning alert-white rounded'>
                            <div class='icon'><i class='fa fa-warning'></i></div>
                            <strong>Attention !</strong> L'email du référent ne peut pas être votre email.
                        </div>";

        } else if (mail($receiver, $subject, $body, $sender)) { // Envoi de l'email
            $send_msg = "<div class='alert alert-success alert-white rounded'>
                            <div class='icon'><i class='fa fa-check'></i></div>
                            <strong>Confirmation</strong> Votre lien a été envoyé à <span style='border-bottom: 1px dashed #999; text-decoration: none;'>$receiver</span>
                        </div>";

        } else {
            $send_msg = "<div class='alert alert-danger alert-white rounded'>
                            <div class='icon'><i class='fa fa-times-circle'></i></div>
                            <strong>Erreur</strong> L'email n'a pas pu être envoyé à <span style='border-bottom: 1px dashed #999; text-decoration: none;'>$receiver</span>
                        </div>";
        }
    }
?>
    ...
    <body>
        ...
        <section class="young">
            ...
            <div class="medium-container">
                ...
                <div class="ref-info">
                    <p><i class="fa-solid fa-cake-candles color-icn"></i> <?php setlocale(LC_TIME, 'fr_FR.utf8','fra'); echo utf8_encode(strftime("%d %B %Y", strtotime($userData['birth']))); ?></p>
                    <p><i class="fa-solid fa-at color-icn"></i> <?php echo $userData['email']; ?></p>
                    <p><i class="fa-solid fa-phone color-icn"></i> <?php echo implode('.', str_split($userData['tel'], 2)); ?></p>
                </div>
